...
export const VerifySportProfile = () => {
  const { id } = useParams();
  ...
  return (
    <div className="px-[6vw] py-10">
      ...
      <div className="mt-8 bg-white p-6 shadow flex flex-col gap-6">
        {verificationPending && candidature ? (
          <div className="bg-green-50 rounded shadow border border-green-400 text-green-500 flex items-center gap-4 p-4">
            ...
            />
            <div>
              <div>Félicitations, vous passez à la phase suivante.</div>
              <div className="flex gap-8">
                <span>
                  <b>Centre de composition</b> : {candidature.center.label}{' '}
                </span>
                <span>
                  <b>Salle</b> : {candidature.room}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-red-50 rounded shadow border border-red-400 text-red-500 flex items-center gap-4 p-4">
            <Icon
              ...
              size={42}
            />
            <div>Désolé, vous n'avez pas été accepté(e).</div>
          </div>
        ) : (
          <div></div>
        )}
